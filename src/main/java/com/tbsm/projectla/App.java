...
public class App {
	...
	public static void main( String[] args ) {
		...
		// Find All Rows with Dates in them
		Map<String, String> foundStuff = new HashMap<>();
		for(String text : rows) {
			String foundIt = new FieldUtility().getDate(text);
			if( foundIt != null ) {
				foundStuff.put(foundIt, text);
			}
		}
				
		// Print out rows with Dates in them
		System.out.println(" ************************************* ");
		foundStuff.forEach((found, text) -> {
			System.out.println("Found: " + found + " --- " + text);
		});
		
		// Search for Dates in the Document and add to List
		// Process Document to Dates
		Dates dates = new FindDates(foundStuff).processDates();
